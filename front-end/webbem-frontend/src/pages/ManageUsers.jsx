...
const ManageUsers = () => {
    ...
    const myUkmId = localStorage.getItem('ukm_id'); // Ambil UKM si Admin
    ...
    const [formData, setFormData] = useState({
        ...
        password: '',
        role_id: '3',
        ukm_id: myRole === 'super_admin' ? '' : myUkmId // Default otomatis untuk admin
    });
    ...
    const handleSubmit = async (e) => {
        ...
        // Paksa UKM ID untuk Admin (Keamanan Frontend)
        if (myRole !== 'super_admin') {
            payload.ukm_id = myUkmId;
        }
        ...
        try {
            ...
            // Reset form
            setFormData({
                name: '', username: '', nia: '', password: '', role_id: '3',
                ukm_id: myRole === 'super_admin' ? '' : myUkmId
            });
    ...
    // Filter Role Dropdown: Admin gak bisa bikin Super Admin
    const roleOptions = [
        { value: '3', label: 'Member / Anggota' },
        { value: '2', label: 'Admin UKM' }
    ];
    if (myRole === 'super_admin') {
        roleOptions.push({ value: '1', label: 'Super Admin' });
    }
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 md:p-8 font-sans text-slate-800 relative pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                {/* Tombol Tambah User sekarang tersedia untuk Admin juga */}
                <button
            ...
            {showAddForm && (
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-blue-100 shadow-xl mb-8 animate-in slide-in-from-top-4 fade-in duration-300">
                    ...
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        {/* Hanya Super Admin yang bisa milih UKM. Admin otomatis dikunci */}
                        {myRole === 'super_admin' && formData.role_id !== '1' && (
                            <CustomDropdown label="UKM" options={ukmOptions} value={formData.ukm_id} onChange={(v) => setFormData({ ...formData, ukm_id: v })} icon={Building} />
                        )}
