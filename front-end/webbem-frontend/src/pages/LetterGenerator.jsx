...
import {
    FileText, Printer, Plus, Trash2, Settings, X,
    Type, Layout, Bold, Italic, Underline, AlignCenter,
    Building2, Lock, ShieldCheck, Eye, EyeOff, User
} from 'lucide-react';
...
const UKM_PRESETS = [
    { id: 9, name: 'BADAN EKSEKUTIF MAHASISWA (BEM)', campus: 'STIKOM POLTEK CIREBON', address: 'Jl. Pusri No. 01 Kedawung, Cirebon, Jawa Barat, 45153', logo: logoBEM, showCampusLogo: true, swapLogos: true },
    { id: 15, name: 'DEWAN PERWAKILAN MAHASISWA (DPM)', campus: 'STIKOM POLTEK CIREBON', address: 'Jl. Pusri No. 01 Kedawung, Cirebon, Jawa Barat, 45153', logo: logoDPM, showCampusLogo: true, swapLogos: true },
    { id: 14, parentOrg: 'MAHASISWA PECINTA ALAM', name: 'MATA ALAM', campus: 'STIKOM POLTEK CIREBON', address: 'Sekretariat Kampus I Jln. Pusri No. 01 ByPass Kedawung, Cirebon Jawa Barat 45153. Telp (0231) 486475 Ext 82/83', logo: logoMataAlam, showCampusLogo: false, swapLogos: false },
    { id: 5, name: 'STIKOM CIREBON CHOIR (SCC)', campus: 'STIKOM POLTEK CIREBON', address: 'Jln. Brigjend Darsono No. 33 Cirebon. Phone (0231) 486475', logo: logoChoir, showCampusLogo: false, swapLogos: false },
...
const RichTextEditor = ({ label, value, onChange, rows = 3 }) => {
    ...
    // Sync value dari luar HANYA jika isinya berbeda dengan yang di dalam editor
    // Ini mencegah kursor loncat ke depan (typing backwards)
    useEffect(() => {
    ...
    const handleFormat = (command) => {
        ...
        // Trigger onChange setelah format agar state tersinkron
    ...
    return (
        <div className="space-y-1">
            ...
            <div
                ref={editorRef}
                className="input-field overflow-y-auto custom-scrollbar outline-none focus:ring-2 focus:ring-blue-500 transition-all"
                style={{ minHeight: `${rows * 20}px` }}
                contentEditable
                suppressContentEditableWarning
                onInput={(e) => onChange(e.currentTarget.innerHTML)}
            // Hapus dangerouslySetInnerHTML dari sini karena sudah dihandle oleh useEffect & Ref
            />
        </div>
    );
};
...
const LetterGenerator = () => {
    ...
    const [settings, setSettings] = useState({
        paperSize: 'A4', fontSize: 12, lineHeight: 1.5, marginTop: 10, marginBottom: 25, fontFamily: 'Times New Roman'
    });
    ...
    const handleVerify = async (e) => {
        ...
        const payload = { username: authInput, password: passwordInput };
        try {
    ...
    // ---------------------------------------------------------------------------------
    // 2. TAMPILAN UTAMA (GENERATOR)
    // ---------------------------------------------------------------------------------
    return (
        <div className="flex flex-col lg:flex-row min-h-screen lg:h-screen bg-gray-100 font-sans text-gray-900 overflow-auto lg:overflow-hidden relative">

            {/* SUNTIKAN CSS PRINT DINAMIS AGAR F4 TIDAK TERPOTONG */}
            <style>
                {`
          @media print {
            @page {
              size: ${settings.paperSize === 'F4' ? '215mm 330mm' : 'A4'};
              margin: 0;
            }
            body { 
              visibility: hidden; 
            ...
            /* Ubah fixed jadi absolute agar halaman bisa memanjang turun */
            #printable-content { 
              visibility: visible; 
              position: absolute; 
              left: 0; 
        ...
        `}
            ...
            {/* --- SIDEBAR FORM (KIRI) --- */}
            <div className="w-full lg:w-[450px] bg-white border-b lg:border-r border-gray-300 flex flex-col shrink-0 lg:h-full print:hidden shadow-xl z-20">

                {/* Header Form */}
                <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50 sticky top-0 z-10">
                    <h2 className="text-lg font-black text-gray-800 flex items-center gap-2">
                        <FileText className="text-blue-600" size={20} /> Generator Surat
                    </h2>
                    <div className="flex gap-2">
                        <button onClick={() => setShowSettings(!showSettings)} className="p-2 rounded-lg hover:bg-gray-200 text-gray-600">
                            <Settings size={20} />
                        </button>
                        <button onClick={handlePrint} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg font-bold text-xs flex items-center gap-2 shadow-lg">
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
                    ...
                    {/* Setting Layout Dropdown */}
                    {showSettings && (
                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 space-y-4 mb-4 shadow-inner">
                            <div className="flex justify-between items-center">
                                <h3 className="text-xs font-bold text-blue-800 uppercase flex items-center gap-1"><Settings size={12} /> Layout Settings</h3>
                                <button onClick={() => setShowSettings(false)}><X size={14} className="text-blue-400" /></button>
                            </div>
                    ...
                    <div className="space-y-5 pb-10">
                        <div className="space-y-2">
                            ...
                            <input name="nomor" placeholder="Nomor Surat" className="input-field" onChange={handleChange} />
                            <input name="perihal" placeholder="Perihal" className="input-field" onChange={handleChange} />
                            <div className="grid grid-cols-2 gap-2">
                                <input name="lampiran" value={form.lampiran} placeholder="Lampiran" className="input-field" onChange={handleChange} />
                                <input name="date" value={form.date} placeholder="Tanggal" className="input-field" onChange={handleChange} />
                            </div>
                        </div>

                        <div className="space-y-2">
                            <Label>Penerima</Label>
                            <input name="tujuan" placeholder="Kepada Yth..." className="input-field" onChange={handleChange} />
                            <input name="tempat" value={form.tempat} placeholder="Di Tempat" className="input-field" onChange={handleChange} />
                        </div>
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center">
                                ...
                                <div className="flex items-center gap-2">
                                    <input type="checkbox" checked={form.useSalam} onChange={(e) => setForm({ ...form, useSalam: e.target.checked })} id="useSalam" />
                                    <label htmlFor="useSalam" className="text-xs text-gray-600 font-bold cursor-pointer">Pakai Salam</label>
                                </div>
                            </div>
                        ...
                        <div className="space-y-3 pt-4 border-t border-gray-100">
                            <div className="flex justify-between items-center">
                                ...
                                <button onClick={addSignatory} className="btn-add"><Plus size={14} /> Tambah</button>
                            </div>

                            <div className="space-y-3">
                                {form.signatories.map((signer, index) => (
                                    <div key={signer.id} className="p-3 bg-gray-50 border border-gray-200 rounded-lg relative group transition hover:bg-white hover:shadow-sm">
                                        <button onClick={() => removeSignatory(signer.id)} className="absolute top-2 right-2 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition">
                                            <Trash2 size={16} />
                                        </button>
                                        <p className="text-[10px] font-bold text-gray-400 mb-1 uppercase">Posisi {index + 1}</p>
                                        <input value={signer.jabatan} onChange={(e) => handleSignatoryChange(signer.id, 'jabatan', e.target.value)} className="input-field-xs font-bold mb-1" placeholder="Jabatan" />
                                        <input value={signer.nama} onChange={(e) => handleSignatoryChange(signer.id, 'nama', e.target.value)} className="input-field-xs mb-1" placeholder="Nama" />
                                        <input value={signer.nia} onChange={(e) => handleSignatoryChange(signer.id, 'nia', e.target.value)} className="input-field-xs" placeholder="NIA" />
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="pt-2">
                                <div className="flex items-center gap-2 mb-2">
                                    <input type="checkbox" checked={form.useMengetahui} onChange={(e) => setForm({ ...form, useMengetahui: e.target.checked })} id="useMengetahui" />
                                    <label htmlFor="useMengetahui" className="text-xs font-extrabold text-blue-600 uppercase tracking-wider cursor-pointer flex items-center gap-1">
                                        <AlignCenter size={12} /> Ada Pihak Mengetahui?
                                    </label>
                                </div>
                                {form.useMengetahui && (
                                    <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                        <input value={form.mengetahui.jabatan} onChange={(e) => setForm({ ...form, mengetahui: { ...form.mengetahui, jabatan: e.target.value } })} className="input-field-xs font-bold mb-1" placeholder="Jabatan (Tengah)" />
                                        <input value={form.mengetahui.nama} onChange={(e) => setForm({ ...form, mengetahui: { ...form.mengetahui, nama: e.target.value } })} className="input-field-xs mb-1" placeholder="Nama Lengkap" />
                                        <input value={form.mengetahui.nia} onChange={(e) => setForm({ ...form, mengetahui: { ...form.mengetahui, nia: e.target.value } })} className="input-field-xs" placeholder="NIA" />
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {/* --- PREVIEW KERTAS (KANAN) --- */}
            <div className="flex-1 bg-gray-200 p-4 lg:p-8 overflow-auto flex justify-center items-start print:p-0 print:bg-white print:block min-h-[60vh] lg:min-h-0 relative">

                {/* Kontainer Kertas agar bisa digeser (scroll) horizontal di HP jika kekecilan */}
                <div className="w-full overflow-x-auto flex justify-center lg:overflow-visible pb-10 print:pb-0">
                    <div id="printable-content" className="bg-white shadow-2xl print:shadow-none relative transition-all duration-300 transform origin-top shrink-0"
                        style={{
                            width: paperDimensions[settings.paperSize].width,
                            minHeight: paperDimensions[settings.paperSize].height,
                        ...
                        <div className="relative z-10">
                            {/* KOP SURAT */}
                            <div className="flex items-center justify-center border-b-4 border-double border-black pb-4 mb-6 gap-4 px-4">
                            ...
                            <div className="flex justify-between items-start mb-6">
                                <div className="w-1/2">
                                    <table className="w-full text-left table-fixed border-collapse">
                                        <tbody>
                                            <tr><td className="w-20 align-top">Nomor</td><td className="w-2 align-top">:</td><td className="break-words">{form.nomor}</td></tr>
                                            <tr><td className="w-20 align-top">Lampiran</td><td className="w-2 align-top">:</td><td className="break-words">{form.lampiran}</td></tr>
                                            <tr><td className="w-20 align-top">Perihal</td><td className="w-2 align-top">:</td><td className="font-bold break-words">{form.perihal}</td></tr>
                                        </tbody>
                                    </table>
                                </div>
                                <div className="text-right w-1/3">
                                    <p>Cirebon, {form.date}</p>
                                </div>
                            </div>
                            ...
                            <div className="mb-6">
                                <p>Yth.</p>
                                <p className="font-bold">{form.tujuan}</p>
                                <p>Di {form.tempat}</p>
                            </div>

                            {/* Isi */}
                            <div className="text-justify space-y-4 mb-8">
                                {form.useSalam && <p>Assalamu'alaikum Wr. Wb.</p>}
                                <p>Dengan hormat,</p>
                                <div className="indent-10" dangerouslySetInnerHTML={{ __html: form.paragraf_pembuka }} />
                                <div className="indent-10" dangerouslySetInnerHTML={{ __html: form.isi_utama }} />
                                <div className="indent-10" dangerouslySetInnerHTML={{ __html: form.penutup }} />
                                {form.useSalam && <p className="mt-4">Wassalamu'alaikum Wr. Wb.</p>}
                            </div>
                            ...
                            <div className="mt-10 w-full px-2">
                                <div className="flex flex-wrap justify-between gap-y-10">
                                    {form.signatories.map((signer, index) => {
                                        const isLastAndOdd = (index === form.signatories.length - 1) && (form.signatories.length % 2 !== 0);
                                        return (
                                            <div key={signer.id} className={`flex flex-col items-center mb-8 break-inside-avoid ${isLastAndOdd ? 'mx-auto w-full' : 'w-[45%]'}`}>
                                                <p className="text-center">{signer.jabatan}</p>
                                                <div className="h-24"></div>
                                                <p className="font-bold underline text-center">{signer.nama}</p>
                                                <p className="text-sm text-center">{signer.nia}</p>
                                            </div>
                                        );
                                    })}
                                </div>
                                {form.useMengetahui && (
                                    <div className="flex flex-col items-center mt-4 w-full break-inside-avoid">
                                        <p className="text-center">Mengetahui,</p>
                                        <p className="text-center">{form.mengetahui.jabatan}</p>
                                        <div className="h-24"></div>
                                        <p className="font-bold underline text-center">{form.mengetahui.nama}</p>
                                        <p className="text-sm text-center">{form.mengetahui.nia}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                {/* Tooltip Mobile */}
                <div className="fixed bottom-4 right-4 bg-black/80 text-white text-[10px] px-3 py-1.5 rounded-full shadow-lg print:hidden backdrop-blur-md z-30 lg:hidden pointer-events-none">
                    Geser untuk melihat kertas 👉
                </div>
            </div>
        </div>
    );
...
// CSS statis yang bersih tanpa @media print agar tidak bentrok dengan suntikan <style> di atas
const css = `
  ...
  .input-field { width: 100%; padding: 10px; background: #fff; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; outline: none; color: #000; transition: border-color 0.2s; }
  .input-field:focus { border-color: #2563eb; outline: 2px solid rgba(37,99,235,0.1); }
  .input-field-sm { width: 100%; padding: 6px; border: 1px solid #d1d5db; border-radius: 6px; font-size: 12px; outline: none; }
  .input-field-sm:focus { border-color: #2563eb; }
  .input-field-xs { width: 100%; padding: 4px 8px; border: 1px solid #e5e7eb; border-radius: 4px; font-size: 12px; background: transparent; outline: none; }
